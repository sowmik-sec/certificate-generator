  ...
  useEffect(() => {
    ...
    const drawAlignmentLines = (aligningObject: any, canvasObjects: any[]) => {
      ...
      canvasObjects.forEach((obj: any) => {
        if (obj === aligningObject || obj.id === 'alignment-line' || obj.id === 'grid-line') return;
        ...
        if (Math.abs(objectCenter.x - objCenter.x) < tolerance) {
          const line = new fabric.Line([
            objCenter.x, 0, 
            objCenter.x, canvasInstance.getHeight() / canvasInstance.getZoom()
          ], {
            stroke: '#ff0000',
            strokeWidth: 1,
            strokeDashArray: [5, 5],
        ...
        if (Math.abs(objectBounds.left - objBounds.left) < tolerance) {
          const line = new fabric.Line([
            objBounds.left, 0,
            objBounds.left, canvasInstance.getHeight() / canvasInstance.getZoom()
          ], {
            stroke: '#ff0000',
            strokeWidth: 1,
            strokeDashArray: [5, 5],
        ...
        if (Math.abs(objectBounds.left + objectBounds.width - (objBounds.left + objBounds.width)) < tolerance) {
          const rightX = objBounds.left + objBounds.width;
          const line = new fabric.Line([
            rightX, 0,
            rightX, canvasInstance.getHeight() / canvasInstance.getZoom()
          ], {
            stroke: '#ff0000',
            strokeWidth: 1,
            strokeDashArray: [5, 5],
        ...
        if (Math.abs(objectCenter.y - objCenter.y) < tolerance) {
          const line = new fabric.Line([
            0, objCenter.y,
            canvasInstance.getWidth() / canvasInstance.getZoom(), objCenter.y
          ], {
            stroke: '#ff0000',
            strokeWidth: 1,
            strokeDashArray: [5, 5],
        ...
        if (Math.abs(objectBounds.top - objBounds.top) < tolerance) {
          const line = new fabric.Line([
            0, objBounds.top,
            canvasInstance.getWidth() / canvasInstance.getZoom(), objBounds.top
          ], {
            stroke: '#ff0000',
            strokeWidth: 1,
            strokeDashArray: [5, 5],
        ...
        if (Math.abs(objectBounds.top + objectBounds.height - (objBounds.top + objBounds.height)) < tolerance) {
          const bottomY = objBounds.top + objBounds.height;
          const line = new fabric.Line([
            0, bottomY,
            canvasInstance.getWidth() / canvasInstance.getZoom(), bottomY
          ], {
            stroke: '#ff0000',
            strokeWidth: 1,
            strokeDashArray: [5, 5],
    ...
    const clearAlignmentLines = () => {
      const lines = canvasInstance.getObjects().filter((obj: any) => obj.id === 'alignment-line');
    ...
    if (snapToObjects) {
      canvasInstance.on('object:moving', (e: any) => {
        ...
        const canvasObjects = canvasInstance.getObjects().filter((obj: any) => 
          obj !== movingObject && obj.id !== 'alignment-line' && obj.id !== 'grid-line'
        );
